...
use crate::ecdsa::utilities::zk_pdl::{PDLStatement, PDLWitness, Prover, Verifier};
...
fn test_zk_pdl() {
  ...
  let (prover_message1, prover_state) = Prover::message1(&witness, &statement, &verifier_message1);
  let verifier_message2 = Verifier::message2(&prover_message1, &statement, &mut verifier_state).expect("");
  let prover_message2 = Prover::message2(&verifier_message1, &verifier_message2, &witness, &prover_state).expect("");
